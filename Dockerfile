...
RUN echo "0 * * * * /bin/bash -c scan > /proc/1/fd/1 2>&1" >> /etc/cron.d/crontab
RUN echo "10 * * * * /bin/bash -c process > /proc/1/fd/1 2>&1" >> /etc/cron.d/crontab
RUN echo "5,15,20,25,30,35,40,45,50,55 * * * * /bin/bash -c move > /proc/1/fd/1 2>&1" >> /etc/cron.d/crontab
...
RUN echo "alias move='cd /app && /usr/local/bin/python3 MoveVideo.py > /proc/1/fd/1 2>&1 && /usr/local/bin/python3 ManageQBittorent.py > /proc//fd/1 2>&1'" >> /root/.bashrc
